...
const ProjectBar: React.FC = () => {
  ...
  return (
    <nav className="w-16 bg-gray-900 flex-shrink-0 flex flex-col items-center py-4 gap-4 border-r border-gray-700">
      ...
      {isCreatingProject ? (
        <div className="flex flex-col items-center gap-2 p-2 bg-gray-800 rounded-lg">
          <input
            type="text"
            value={newProjectName}
            onChange={(e) => setNewProjectName(e.target.value)}
            placeholder="Project name"
            className="w-20 px-2 py-1 text-xs bg-gray-700 text-white rounded border-none outline-none"
            autoFocus
            onKeyDown={(e) => {
